...
class _SignupScreenState extends State<SignupScreen> {
  ...
  Future<void> _handleSignup() async {
    ...
    if (_emailController.text.trim().isEmpty) {
      _showError(context.tr('please_enter_email'));
      return;
    }

    if (!RegExp(r'^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$').hasMatch(_emailController.text)) {
      _showError(context.tr('please_enter_valid_email'));
      return;
    }

    // Validate username
    if (_usernameController.text.trim().isEmpty) {
      _showError(context.tr('please_enter_username'));
      return;
    }

    if (!RegExp(r'^[a-zA-Z0-9_]{3,20}$').hasMatch(_usernameController.text)) {
      _showError(context.tr('username_requirements'));
      return;
    }
try {
    ...
    if (existingUser != null) {
      _showError(context.tr('username_already_taken'));
      return;
    }
  } catch (e) {
  ...
  try {
    final db = DatabaseHelper.instance;
    final existingEmail = await db.getUserByEmail(_emailController.text.trim());
    ...
    if (_passwordController.text != _confirmPasswordController.text) {
      _showError(context.tr('passwords_do_not_match'));
      return;
    }
                                ...
                                child: Row(
                                  ...
                                  children: [
                                    ...
                                    Text(
                                      ...
                                      style: TextStyle(
                                        fontSize: 14,
                                        fontWeight: _selectedRole == 'participant'
                                            ? FontWeight.bold
                                            : FontWeight.normal,
                                        color: _selectedRole == 'participant'
                                            ? const Color.fromARGB(255, 66, 22, 79)
                      ...
                      child: ElevatedButton(
                        ...
                        child: Text(
                          context.tr('sign_up_as_role').replaceAll(
                            '{role}', 
                            _selectedRole == 'participant' ? context.tr('participant') : context.tr('organization')
                          ),
                          style: const TextStyle(
                            fontSize: 16,
                            fontWeight: FontWeight.w600,
