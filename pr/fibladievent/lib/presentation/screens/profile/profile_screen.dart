...
import '/../presentation/screens/organization/organization_info_screen.dart';
...
class _ProfileScreenState extends State<ProfileScreen>
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.resumed) {
      print('Profile screen resumed - refreshing stats');
      _loadUserStats();
    }
  }
  ...
  Future<void> _loadUserStats() async {
    ...
    if (userState is UserLoaded) {
      try {
        ...
        final favResp = await _apiService.get('/favorites');
        final favoritesList =
            List<Map<String, dynamic>>.from(favResp.data as List? ?? []);

        int eventsCountValue = 0;

        if (user.role == 'organization') {
          // For organizers: count events they created
          final createdResp = await _apiService.get('/events/my-events');
          final createdList =
              List<Map<String, dynamic>>.from(createdResp.data as List? ?? []);
          eventsCountValue = createdList.length;
        } else {
          // For participants: count events they joined/registered for
          final joinedResp = await _apiService.get('/events/joined-events');
          final joinedList =
              List<Map<String, dynamic>>.from(joinedResp.data as List? ?? []);
          eventsCountValue = joinedList.length;
        }
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              try {
                // Call API logout and clear stored token
                ...
                // Ensure token is cleared on error as well
                await _apiService.clearToken();
                if (context.mounted) {
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Row(
                            children: [
                              if (user.role == 'organization') ...[
                                ...
                                // For participants: only Favorites
                                Expanded(
                                  child: _buildStatCard(
                                    icon: Icons.favorite,
                                    title: context.tr('favorites'),
                                ...
                                TextButton.icon(
                                  onPressed: () {
                                    _showEditCategoriesDialog(
                                        context, user.selectedCategories);
                                  },
                          ...
                          _buildSettingsItem(
                            ...
                            subtitle:
                                '${user.selectedCategories.length} ${context.tr('interests').toLowerCase()}',
                            color: Colors.orange,
  ...
  void _showEditCategoriesDialog(
      BuildContext context, List<String> currentCategories) {
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                if (!codeSent) ...[
                  Text(
                    context.tr('delete_account_instructions'),
                  ...
                  ElevatedButton(
                            ...
                            try {
                              ...
                              if (context.mounted) {
                                ...
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(
                                    content: Text(
                                        context.tr('verification_code_sent')),
                                    backgroundColor: Colors.green,
                                  ),
                                );
                              }
                    ...
                    child: isLoading
                        ...
                        : Text(context.tr('send_verification_code')),
                  ),
                ] else ...[
                  TextField(
                    ...
                    : () async {
                        if (verificationCodeController.text.length != 6) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content:
                                  Text(context.tr('please_enter_6_digit_code')),
                              backgroundColor: Colors.red,
                            ),
                          );
                          return;
