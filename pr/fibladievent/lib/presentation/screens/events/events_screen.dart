  ...
  if (userState is UserLoaded) {
    ...
    try {
      ...
      if (userRole == 'organization') {
        ...
        final upcomingResponse = await _apiService.get('/events/organized/upcoming');
        final pastResponse = await _apiService.get('/events/organized/past');

        print('Upcoming JSON: ${upcomingResponse.data}');
        print('Past JSON: ${pastResponse.data}');

        upcoming = (upcomingResponse.data['events'] as List? ?? [])
            .map((e) => Event.fromJson(e as Map<String, dynamic>))
            .toList();
        past = (pastResponse.data['events'] as List? ?? [])
            .map((e) => Event.fromJson(e as Map<String, dynamic>))
            .toList();
      } else {
        // Events joined by participant
        final upcomingResponse = await _apiService.get('/events/my-events/upcoming');
        final pastResponse = await _apiService.get('/events/my-events/past');
              ...
              Positioned(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    shape: BoxShape.circle,
                    border:
                        Border.all(color: const Color(0xFFF5F3FF), width: 4),
                  ),
                  child: CustomPaint(
          ...
          Text(
            isUpcomingSelected
                ? (userRole == 'organization'
                    ? AppLocalizations.of(context)!
                        .translate('create_events_to_see_them_here')
                    : AppLocalizations.of(context)!
                        .translate('join_events_to_see_them_here'))
                : (userRole == 'organization'
                    ? AppLocalizations.of(context)!
                        .translate('your_created_events_will_appear_here')
                    : AppLocalizations.of(context)!
                        .translate('your_attended_events_will_appear_here')),
            style: TextStyle(
              fontSize: 16,
